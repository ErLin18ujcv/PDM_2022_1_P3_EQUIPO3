...
class UsuarioBusiness: IUsuarioBusiness {
    ...
    override fun saveUsuario(usuario: Usuario): Usuario {
        try {
            //nombre
            usuario.nombre= usuario.nombre.lowercase()
            ...
            //if (!usuario.nombre.trimStart().trimEnd().matches(Regex("^([A-ZÁÉÍÓÚ]{1}[a-záéíóú]*)|\\s[A-ZÁÉÍÓÚ]{1}[a-záéíóú]*")))
            //throw BusinessException("Solo puede ingresar maximo 2 nombres y cada uno debe iniciar en mayusculas.")
             var nombre=""
            ...
            //apellido
            usuario.apellido=usuario.apellido.lowercase()
    ...
    override fun updateUsuario(usuario: Usuario): Usuario {
        ...
        if (!opt.isPresent)
            ...
            try {
                ...
                //fecha cambio contrasena
                if (usuario.fechaCambioContrasena!!.isAfter(LocalDate.now()))
                    throw BusinessException("La fecha de cambio de contrasena no puede ocurrir despues de la fecha actual.")
    ...
    override fun getUsuarioByNombre(nombreUsuario: String): Usuario {
        ...
        if (!opt.isPresent)
            throw NotFoundException("No se encontro nombre del Usuario $nombreUsuario")
        return opt.get()
    }
    @Throws(BusinessException::class,NotFoundException::class)
    override fun getUsuarioByApellido(apellidoUsuario: String): Usuario {
        ...
        if (!opt.isPresent)
            throw NotFoundException("No se encontro apellido del Usuario $apellidoUsuario")
        return opt.get()
    }
    ...
    override fun getUsuarioByCorreo(correoUsuario: String): Usuario {
        ...
        if (!opt.isPresent)
            throw NotFoundException("No se encontro el correo $correoUsuario")
        return opt.get()
    }
    @Throws(BusinessException::class, NotFoundException::class)
    override fun getUsuarioByFechaNacimiento(fechaNacimientoUsuario: LocalDate?): Usuario {
        ...
        if (!opt.isPresent)
            throw NotFoundException("No se encotro el fecha de Nacimiento $fechaNacimientoUsuario")
        return opt.get()
    }
